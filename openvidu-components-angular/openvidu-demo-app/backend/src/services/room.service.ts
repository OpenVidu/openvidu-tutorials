...
import { LIVEKIT_API_KEY, LIVEKIT_API_SECRET, LIVEKIT_URL_PRIVATE, DEMO_APP_NAME_ID } from '../config.js';
import { OpenViduDemoAppError, errorRoomNotFound, internalError } from '../models/error.model.js';
...
export class RoomService {
	...
	async isRoomCreatedByMe(roomOrRoomName: Room | string): Promise<boolean> {
		try {
			...
			return metadata?.createdBy === DEMO_APP_NAME_ID;
		} catch (error) {
	 ...
	 * @throws {OpenViduDemoAppError} If there is an error sending the signal.
