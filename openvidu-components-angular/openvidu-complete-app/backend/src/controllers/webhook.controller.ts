...
export const webhookHandler = async (req: Request, res: Response) => {
	try {
		...
		// Skip webhook events that are not related to OpenVidu
		if (roomOrRoomName && !isRoomCreatedByMe) {
			logger.verbose(`Skipping webhook, event is not related to OpenVidu: ${eventType}`);
...
function getRoomOrRoomName(webhookEvent: WebhookEvent) {
	...
	// This not affect OpenVidu, but it is a limitation of the LiveKit server
